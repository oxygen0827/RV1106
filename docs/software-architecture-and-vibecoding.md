...
RV1106 为单核 A7；资料标注 256 MB 内存，但当前固件历史实测 Linux `MemTotal` 约 55 MB，应用预算必须以实机为准（见 `AGENTS.md`）。板端适合显示、触摸、唤醒词、Opus、简单业务逻辑和量化视觉模型；语音理解和生成默认放在电脑/服务器。当前 AIChat 使用 VAD 分段和智谱 `GLM-4-Voice` 端到端处理，板端继续只负责 16 kHz Opus 采集/播放。
